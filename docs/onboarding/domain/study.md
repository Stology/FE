...
노드는 `KnowledgeConceptNode`(온톨로지 개념)와 `KnowledgeMaterialNode`(자료)로 나뉘며, 자료가 `evidence` 엣지로 개념에 연결되면 해당 개념이 활성화됩니다. 개념은 연결된 자료 중 가장 이른 주차를 `activatedWeek`(신규 활성)로, 그 이후 주차를 `reinforcedWeeks`(보강)로 구분해 추적합니다. 활성 노드는 연결된 자료 수가 많을수록 더 진한 색으로 강조하며, 활성 노드와 비활성 노드를 함께 표시합니다.

그래프 동작은 다음을 포함합니다.

- 노드 유형: `KnowledgeConceptNode`(개념)와 `KnowledgeMaterialNode`(자료)를 함께 렌더링
- 활성화: 자료 수가 많을수록 진한 색으로 강조, 신규 활성/보강 상태 구분
- 검색/필터: 노드명 검색은 필터와 무관하게 전체 노드를 대상으로 하며, 주차 필터를 선택하면 해당 주차에 신규 활성 또는 보강 활동이 있는 노드만 남김
- ego layout: 노드를 클릭하면 그 노드를 중심으로 연관 노드가 재정렬되는 ego layout으로 전환되고 카메라가 자동으로 프레이밍
- 관계 토글: 노드 상세의 연결 관계는 `ConceptRelationKind`의 기반(`based-on`)·맥락(`associated-with`)·확장(`advanced-from`)·대조(`contrasted-with`) 중 하나만 선택하는 세그먼트 토글이며, 연결 노드명을 클릭하면 그 노드 상세로 전환
- 원본 자료: 노드 상세의 `[원본 자료 보기]`를 선택하면 관련 자료 목록 팝업이 열리며, 팝업의 다운로드 액션을 선택하면 해당 스터디의 자료 업로드 탭으로 이동하고 `materialId`를 쿼리로 유지합니다.
