...
> Three.js 기반 3D WebGL 렌더링으로 전환 중입니다(#51 후속 작업). 현재는 데이터 모델만 전환되었고, 화면은 접근성용 DOM 노드 목록(`KnowledgeNodeListFallback`)이 그래프 자리를 임시로 대신합니다. 렌더링/상호작용 서술은 3D 렌더러 도입 후 확정합니다.

노드는 `KnowledgeConceptNode`(온톨로지 개념)와 `KnowledgeMaterialNode`(자료)로 나뉘며, 자료가 `evidence` 엣지로 개념에 연결되면 해당 개념이 활성화됩니다. 개념은 연결된 자료 중 가장 이른 주차를 `activatedWeek`(신규 활성)로, 그 이후 주차를 `reinforcedWeeks`(보강)로 구분해 추적합니다. 활성 노드와 비활성 노드를 함께 표시합니다. 노드명 검색은 필터와 무관하게 전체 노드를 대상으로 합니다. 주차 필터를 선택하면 해당 주차에 신규 활성 또는 보강 활동이 있는 노드만 남깁니다. 노드 패널의 연결 관계는 `ConceptRelationKind`의 기반(`based-on`)·맥락(`associated-with`)·확장(`advanced-from`)·대조(`contrasted-with`) 중 하나만 선택하는 세그먼트 토글이며, 연결 노드명을 클릭하면 그 노드 상세로 패널을 전환합니다.
